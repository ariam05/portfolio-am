    ...
    return (
        <nav className='fixed top-0 left-0 right-0 z-10 bg-[#0d0628] bg-opacity-100 py-3'>
            <div className='flex flex-wrap items-center justify-between mx-auto px-4 py-2'>
                <Link href={"/"} className='text-10 md:text-15 text-white font-semibold ml-10'>Ariam Mahray</Link>
                        ...
                        {navLinks.map((link, index) => (
                            <li key={index}>
                                <a 
                                    href={link.path} 
                                    onClick={() => {
                                        setMenu(link.title.toLowerCase());
                                        setNavbarOpen(false); 
                                    }} 
                                    className={`text-white hover:text-purple-600 mr-5 ${menu === link.title.toLowerCase() ? "active" : ""}`}
